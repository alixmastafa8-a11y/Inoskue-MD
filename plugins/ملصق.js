...
import { addExif } from "../lib/ملصق.js";
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp/g.test(mime)) {
      ...
      throw `Reply an image/video/ملصق with command ${usedPrefix + command}`;
  } catch (e) {
    console.log(e);
    stiker = e;
  } finally {
...
handler.help = ['ملصق'];
handler.tags = ["ملصق"];
handler.command = /^(sfull|sgif|s|ملصق|stiker)$/i;
handler.limit = 1
...
async function createSticker(img, url, packName, authorName, quality) {
  ...
  return new Sticker(img ? img : url, ملصقMetadata).toBuffer();
}
async function mp4ToWebp(file, ملصقMetadata) {
  if (ملصقMetadata) {
    if (!ملصقMetadata.pack) ملصقMetadata.pack = "‎";
    if (!ملصقMetadata.author) ملصقMetadata.author = "‎";
    if (!ملصقMetadata.crop) ملصقMetadata.crop = false;
  } else if (!ملصقMetadata) {
    ملصقMetadata = { pack: "‎", author: "‎", crop: false };
  }
  let getBase64 = file.toString("base64");
  const Format = {
    file: `data:video/mp4;base64,${getBase64}`,
    processOptions: {
      crop: ملصقMetadata?.crop,
    ...
    config: {
      ...
      ملصقServerEndpoint: true,
    },
  };
  let res = await fetch(
    "https://ملصق-api.openwa.dev/convertMp4BufferToWebpDataUrl",
    {
      method: "post",
      headers: {
