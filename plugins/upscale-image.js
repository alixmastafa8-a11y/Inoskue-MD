...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    if (!/image/.test(mime)) {
      return m.reply(`Send/رد علىn image with caption ${usedPrefix + command}`)
    }
    m.reply('⏳ جارٍ المعالجة image, please wait...')
    let media = await quoted.download()
    let result = await hdr(media, 4)
    await conn.sendFile(m.chat, result, 'hdr.png', '✅ Here is the result', m)
  } catch (err) {
    console.log(err)
    m.reply('❌ فشل to process image.')
  }
}
...
async function getToken() {
  try {
    ...
    throw new Error('Token خطأ: ' + err.message)
  }
}
