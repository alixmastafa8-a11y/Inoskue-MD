...
رد على ملصق or audio message with:
${usedPrefix + command}
مثال:
• رد على ملصق → type *${usedPrefix + command}*
• رد علىn audio → type *${usedPrefix + command}*
`;
  }
  m.react("⏳");
  let mime = m.quoted.mimetype || "";
  if (!/webp|audio/.test(mime)) {
    throw `Please reply to a *ملصق* or *audio* message with caption *${usedPrefix + command}*`;
  }
  ...
  try {
    ...
    if (!out || out.length === 0) throw "فشل to generate video.";
    m.react("✅");
    await conn.sendFile(m.chat, out, `video-${Date.now()}.mp4`, "🎬 Conversion نجحful!", m);
  } catch (err) {
    console.error(err);
    m.react("❌");
    throw "فشل to convert to video.";
  }
};
