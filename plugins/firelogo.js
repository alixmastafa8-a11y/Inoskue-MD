...
let handler = async (m, { conn, args }) => {
  try {
    ...
    if (!text) return conn.reply(m.chat, 'يرجى تقديم text to generate the logo.', m);
    const params = {
    ...
    if (!imageUrl) return conn.reply(m.chat, 'فشل to retrieve the image URL.', m);
    await conn.sendMessage(m.chat, { image: { url: imageUrl }, caption: `Logo for "${text}"` }, { quoted: m });
  } catch (error) {
    console.error(error);
    conn.reply(m.chat, 'حدث خطأ while generating the logo.', m);
  }
};
