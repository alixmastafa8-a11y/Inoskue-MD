...
let handler = async (m, { conn, args }) => {
  if (args.length < 3) {
    return m.reply('✳️ يرجى إدخال 3 text segments separated by commas:\n\nمثال: .textcraft Hello, My Bot, inoskue')
  }
  let [text1, text2, text3] = args.join(' ').split(',')
  try {
    m.reply('⏳ يرجى الانتظار while I generate your image...')
    ...
    console.error(err)
    m.reply('❌ فشل to generate image. Please try again later.')
  }
}
