...
async function fetchUrlData(targetUrl, options = {}) {
  if (!targetUrl || typeof targetUrl !== "string")
    throw new Error("❌ يرجى تقديم a valid link.");
...
let handler = async (m, { conn, text }) => {
  if (!text) return m.reply("🔗 يرجى تقديم a valid Twitter/X link.");
  m.reply("⏳ يرجى الانتظار... downloading the video.");
  try {
    ...
    if (result?.formats?.length) {
      ...
      m.reply("⚠️ فشل to extract video link. Try another URL.");
    }
  } catch (err) {
    m.reply(`❌ خطأ: ${err.message}`);
  }
};
handler.help = handler.command = ['اكس'];
