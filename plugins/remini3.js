  ...
  upscale: async (imagePath, scaleRatio, maxRetries = 30, retryDelay = 2000) => {
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      if (statusResult.data.status === "error") {
        throw new Error("جارٍ المعالجة failed on server");
      }
      await new Promise((resolve) => setTimeout(resolve, retryDelay));
    }
    throw new Error("جارٍ المعالجة timeout - maximum retries exceeded");
  },
};
const handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if (!m.quoted || !/image/.test(m.quoted.mimetype || "")) {
    return conn.reply(
      m.chat,
      `رد علىn image with caption *${usedPrefix + command}* 2x or *${usedPrefix + command}* 4x`,
      fkontak
    );
  }
  ...
  try {
    ...
    const errorMessage =
      typeof err === "string"
        ? err
        : "❌ فشل to upscale image. Please make sure the API is working properly.";
