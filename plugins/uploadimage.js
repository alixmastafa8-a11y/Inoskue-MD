  ...
  getToken: async () => {
    try {
      ...
      if (match && match[1]) {
        ...
        throw new Error("فشل to find auth_token in the page.")
      }
    } catch (error) {
      throw new Error(`فشل to get token: ${error.message}`)
    }
  },
  upload: async (filePath) => {
    if (!fs.existsSync(filePath)) {
      throw new Error(`الملف غير موجود at path: ${filePath}`)
    }
    try {
...
let handler = async (m, { conn }) => {
  ...
  try {
    ...
    if (result && result.image && result.image.url) {
      await conn.reply(m.chat, `✅ تم الرفع بنجاح!\n\n🔗 ${result.image.url}`, m)
    } else {
