...
async function creartTxt2Img(prompt) {
  try {
    ...
    throw new Error("❌ فشل to create image (txt2img): " + (err?.response?.data?.message || err.message));
  }
}
async function creartImg2Img(prompt, imageBuffer) {
  ...
  try {
    ...
    throw new Error("❌ فشل to create image (img2img): " + (err?.response?.data?.message || err.message));
  } finally {
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) {
    return m.reply(
      ...
      `2. *Image-to-Image*: رد علىn image with *${usedPrefix + command} make it a watercolor painting*`
    );
  }
  ...
  try {
    if (/image/g.test(mime)) {
      await m.reply('🎨 جارٍ المعالجة your image with the new prompt, please wait...');
