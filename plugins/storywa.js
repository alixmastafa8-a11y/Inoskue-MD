...
let handler = async (m, { conn, args }) => {
  ...
  switch (action) {
    case "search":
      if (!input) return m.reply("❗ يرجى تقديم a search كلمة البحث.");
      result = await api.search({ query: input });
      break;
    case "bycat":
      if (!input) return m.reply("❗ يرجى تقديم a category name.");
    ...
    case "statuscats":
      ...
      break;
    case "download":
      if (!input) return m.reply("❗ يرجى تقديم a video ID to download.");
...
Use English كلمة البحثs for better accuracy.
`;
