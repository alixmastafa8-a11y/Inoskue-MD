const keepv = {
    ...
    async getCookieAndRedirectUrl(origin, baseHeaders) {
        try {
            ...
            if (!cookieHeader) throw Error(`'set-cookie' header was غير موجود in the response.`);
            const cookie = cookieHeader.split(';')[0];
            if (!cookie) throw Error(`فشل to parse the cookie from the 'set-cookie' header.`);
    ...
    async checkJob(resultValidateCookie, resultConvert, origin, baseHeaders, format, identifier, progressCallback) {
        ...
        for (let fetchCount = 0; fetchCount < MAX_FETCH_ATTEMPT; fetchCount++) {
            ...
            if (message) {
                if (message.startsWith("جارٍ التحميل audio data")) {
...
handler.help = ['تحميل-فيد'];
handler.command = ['تحميل-فيد'];
