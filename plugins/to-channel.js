async function handler(m, { conn, text }) {
    try {
        ...
        const idsal = '120363285847738492@newsletter'; 
        ...
        if (containsBannedWord) {
            ...
            try {
                ...
                await conn.reply(m.chat, 'فشل to kick user, check bot permissions.', m);
            }
            return;
        }
        let ppuser;
        ...
        const ctx = {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: idsal,
                serverMessageId: 20,
                newsletterName: 'inoskue Ai | أسطورة البوتات '
            },
            externalAdReply: {
                title: pushname,
                body: `inoskue runtime is active.| سيلانا رهن الاشارة`,
                thumbnailUrl: ppuser,
                mediaType: 1,
        ...
        if (isMedia) {
            const media = await m.quoted.download(true); 
            if (/image/.test(type)) {
            ...
            } else if (/ملصق/.test(type)) {
                await conn.sendMessage(idsal, { ملصق: { url: media }, contextInfo: ctx });
