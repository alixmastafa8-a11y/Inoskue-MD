...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!mime || !mime.includes("image")) return m.reply(
    `*❗Send/رد علىn image with caption:*\n> ${usedPrefix + command} [prompt]`
  );
  ...
  try {
    ...
    let errorMsg = e.response
      ...
      : e.message;
    return m.reply(`فشل to upload image to tmpfiles.org.\n*Error:* ${errorMsg}`);
  }
  let taskId;
  try {
    ...
    if (!taskId) return m.reply("❌ فشل to get taskId from veo31ai.io. ('gen' API did not return taskId)");
  } catch (e) {
    conn.sendMessage(m.chat, { react: { text: "❗", key: m.key } });
    let errorMsg = e.response
      ? `Server error: ${e.response.status} - ${JSON.stringify(e.response.data)}`
      : e.message;
    return m.reply(`فشل to start video generation process.\n*Error:* ${errorMsg}`);
  }
  try {
    conn.sendMessage(m.chat, { 
      text: `✅ Task successfully created
(Task ID: ${taskId}).
⏳ جارٍ المعالجة video... This may take 3–5 minutes. يرجى الانتظار.` 
    }, { quoted: fkontak });
    let videoUrl;
    ...
    if (!videoUrl) return m.reply("❌ فشل to retrieve video. Timeout reached (5 minutes) or server failed.");
    ...
    let errorMsg = e.response
      ? `Server error: ${e.response.status} - ${JSON.stringify(e.response.data)}`
      : e.message;
    m.reply(`حدث خطأ while waiting for or retrieving the video.\n*Error:* ${errorMsg}`);
  }
};
