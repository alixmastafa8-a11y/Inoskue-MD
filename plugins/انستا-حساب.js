...
async function StalkIg(username) {
  ...
  try {
    ...
    if (!raw || profileRes.data.status !== 'success') throw new Error('فشل to fetch profile info');
...
let handler = async (m, { conn, args }) => {
  if (!args[0]) return m.reply('❗ يرجى تقديم an Instagram username.\nمثال: .ig-profile noureddine_ouafy');
  const res = await StalkIg(args[0].replace(/^@/, ''));
  if (res.error) return m.reply(`❌ خطأ: ${res.message}`);
  const caption = `
📛 Name: ${res.name}
...
handler.help = handler.command = ['انستا-حساب'];
handler.tags = ['tools'];
handler.limit = true;
