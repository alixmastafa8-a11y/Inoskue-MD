...
const gemini = {
  getNewCookie: async function () {
    ...
    if (!cookieHeader) throw new Error('فشل to retrieve Gemini cookie.');
  ...
  ask: async function (prompt, previousId = null) {
    ...
    if (!found) throw new Error("فشل to parse Gemini response.");
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) throw `*مثال:* ${usedPrefix + command} on/off`;
