    ...
    try {
        if (!text) {
            let guide = `⚡ *WhatsApp Groups Search Guide*\n\n` +
                        `✏️ *Command:* .searchgroups <كلمة البحثs>\n` +
                        `📌 You can enter one or more كلمة البحثs separated by commas.\n\n` +
                        `مثال:\n` +
                        `.searchgroups anime, games, movies\n\n` +
                        `💡 The bot will search for groups matching these كلمة البحثs and display the name, description, and invite link.`;
        ...
        const results = [];
        const كلمة البحثList = text.split(',');
        for (const name of كلمة البحثList) {
            const كلمة البحث = name.trim();
            let loop_count = 0;
            while (loop_count < 10) {
                const data = new URLSearchParams({
                    group_no: `${loop_count}`,
                    search: true,
                    كلمة البحث: كلمة البحث,
        ...
        if (results.length === 0) return m.reply("⚠️ No groups found for these كلمة البحثs.");
        ...
        console.error(err);
        m.reply("❌ حدث خطأ while searching for groups.");
    }
};
