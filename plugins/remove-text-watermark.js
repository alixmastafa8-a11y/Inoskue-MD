...
class TextWatermarkRemover {
  ...
  async queryجارٍ المعالجةStatus(msgId) {
  ...
  async waitForCompletion(msgId) {
    for (let attempt = 0; attempt < 30; attempt++) {
      const result = await this.queryجارٍ المعالجةStatus(msgId);
    ...
    throw new Error("جارٍ المعالجة timeout");
  }
...
let handler = async (m, { conn, text }) => {
  ...
  try {
    await m.reply("⏳ جارٍ المعالجة image... يرجى الانتظار");
