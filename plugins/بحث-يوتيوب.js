...
let handler = async (m, { text, conn }) => {
  if (!text) return m.reply('يرجى إدخال a search query.\nمثال: .youtubesearch cat videos');
  let results = await searchYouTubeTop10(text);
  if (!results) return m.reply('لم يتم العثور على نتائج or an error occurred.');
...
handler.help = handler.command = ['بحث-يوتيوب'];
handler.tags = ['search'];
handler.limit = true;
...
async function searchYouTubeTop10(query) {
  try {
    ...
    if (!scriptTag) throw new Error('ytInitialData script tag غير موجود.');
    const ytInitialDataRaw = $(scriptTag).html().match(/var ytInitialData = (.*?});/);
    if (!ytInitialDataRaw || ytInitialDataRaw.length < 2) throw new Error('فشل to extract ytInitialData.');
    ...
    if (!contents) throw new Error('Search results غير موجود.');
