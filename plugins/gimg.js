...
const googleSearchImage = async (query) => {
  if (!query) throw Error('كلمة البحث must not be empty')
  const usp = {
    "as_st": "y",
    "as_q": query,
    "imgsz": "l",
  ...
  if (!response.ok) throw Error(`فشل to fetch: ${response.status} ${response.statusText}`)
...
let handler = async (m, { conn, args }) => {
  ...
  if (!query) return m.reply('📸 يرجى تقديم a search term. مثال:\n.gimg cat')
  try {
